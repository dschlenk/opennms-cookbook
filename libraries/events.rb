...
module Events
  ...
  def event_changed?(event, node)
    ...
    if event.alarm_data.nil?
      unless event_el.elements['alarm-data'].nil?
        Chef::Log.debug "Event alarm-data present but nil in new resource."
        return true
      end
    end
    if ad_el.nil?
