    ...
    module Provision
      ...
      module ModelImportHttpRequest
        ...
        def model_import(name)
          ...
          find_resource!(:http_request, "opennms_model_import POST #{name}")
        end

        private

          def model_import_exist?(name)
            !find_resource(:http_request, "opennms_model_import POST #{name}").nil?
          rescue Chef::Exceptions::ResourceNotFound
            false
          end
          ...
          def model_import_create(name)
            ...
            with_run_context(:root) do
              declare_resource(:http_request, "opennms_model_import POST #{name}") do
