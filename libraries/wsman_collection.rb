...
module WsmanCollection
  ...
  def created_wsman_collection (current_resource, node)
    Chef::Log.debug "Creating wsman collection : '#{current_resource.name}'"
    file = ::File.new("#{node['opennms']['conf']['home']}/etc/wsman-datacollection-config.xml", 'r')
    contents = file.read
    doc = REXML::Document.new(contents, respect_whitespace: :all)
    doc.context[:attribute_quote] = :quote
    collection_el = doc.root.add_element 'collection', 'name' => current_resource.name
    rrd_el = collection_el.add_element 'rrd', 'step' => current_resource.rrd_step
    current_resource.rras.each do |rra|
    ...
    unless current_resource.include_system_definitions.nil?
      unless current_resource.include_system_definitions == true
        collection_el.add_element 'include-system-definitions'
      end
    end

    unless current_resource.include_system_definition.nil?
      current_resource.include_system_definition.each do |name|
