control 'threshd_service' do
  describe threshd_package('cheftest2') do
    ...
    its('services') { should eq [{ 'name' => 'SNMP', 'interval' => 300_000, 'status' => 'on', 'params' => { 'thresholding-group' => 'cheftest2' } }, { 'name' => 'WS-Man', 'interval' => 300000, 'status' => 'on', 'user-defined' => true, 'params' => { 'thresholding-group' => 'cheftest2' } }, { 'name' => 'WS-Man2', 'interval' => 300000, 'status' => 'on', 'user-defined' => false }] }
  end
  ...
  describe threshd_package('hrstorage') do
    ...
    its('specifics') { should eq nil }
    its('include_ranges') { should eq [{ 'begin' => '1.1.1.1', 'end' => '254.254.254.254' }, { 'begin' => '::1', 'end' => 'ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff' }] }
    its('exclude_ranges') { should eq nil }
    its('include_urls') { should eq nil }
    its('services') { should eq nil }
  end
end
