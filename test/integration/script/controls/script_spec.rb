control 'script' do
  ...
  describe scriptd_script('groovy', 'stop', "def log = bsf.lookupBean(\"log\"); def now = new Date(); log.info(\"Groovy stop script executed at: ${now}\"); if (now.hours > 18) { log.warn(\"Script stopped after hours.\"); }") do # rubocop:disable Style/StringLiterals
    it { should exist }
  end
