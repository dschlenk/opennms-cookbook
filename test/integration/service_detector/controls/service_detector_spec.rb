control 'service_detector' do
  describe foreign_source('another-source', 1237) do
    it { should exist }
  end
  ...
  describe service_detector('ICMP', 'another-source', 1237) do
    it { should exist }
  ...
  describe service_detector('I C M P', 'another-source', 1237) do
    it { should exist }
    its('time_out') { should eq 12_000 }
    its('parameters') { should eq 'ipMatch' => '127.0.0.1' }
  end
