...
#######
# Load the binaryround(value) method from libraries/initdb.rb
...
if node['opennms'].attribute?('config_pgtune') && node['opennms']['config_pgtune'].attribute?('db_type')
  db_type = node['opennms']['config_pgtune']['db_type']
  unless %w(dw oltp web mixed desktop).include?(db_type)
    Chef::Log.fatal([
      "Bad value (#{db_type})",
      "for node['opennms']['config_pgtune']['db_type'] attribute.",
...
if node['opennms'].attribute?('config_pgtune') && node['opennms']['config_pgtune'].attribute?('max_connections')
  max_connections = node['opennms']['config_pgtune']['max_connections'].to_i
  if max_connections <= 0
    Chef::Log.fatal([
      "Bad value (#{max_connections})",
      "for node['opennms']['config_pgtune']['max_connections'] attribute.",
...
if node['opennms'].attribute?('config_pgtune') && node['opennms']['config_pgtune'].attribute?('total_memory')
  total_memory = node['opennms']['config_pgtune']['total_memory']
  if total_memory.match(/\A[1-9]\d*kB\Z/).nil?
    Chef::Application.fatal!([
      "Bad value (#{total_memory})",
      "for node['opennms']['config_pgtune']['total_memory'] attribute.",
...
node.default['opennms']['config']['max_connections'] = con
...
if mem >= 256
  ...
  node.default['opennms']['config']['shared_buffers'] = binaryround(shared_buffers * 1024 * 1024)
  ...
  node.default['opennms']['config']['effective_cache_size'] = binaryround(effective_cache_size * 1024 * 1024)
  ...
  node.default['opennms']['config']['work_mem'] = binaryround(work_mem * 1024 * 1024)
  ...
  node.default['opennms']['config']['maintenance_work_mem'] = binaryround(maintenance_work_mem * 1024 * 1024)
...
if node['opennms']['version'].to_f >= 9.5
  node.default['opennms']['config']['max_wal_size'] = ((3 * checkpoint_segments) * 16).to_s + 'MB'
else
  node.default['opennms']['config']['checkpoint_segments'] = checkpoint_segments
end
...
node.default['opennms']['config']['checkpoint_completion_target'] = checkpoint_completion_target
...
if node['opennms']['version'].to_f < 9.1
  wal_buffers = 512 * checkpoint_segments
  # The pgtune seems to use 1kB units for wal_buffers
  node.default['opennms']['config']['wal_buffers'] = binaryround(wal_buffers * 1024)
else
  node.default['opennms']['config']['wal_buffers'] = '-1'
end
...
node.default['opennms']['config']['default_statistics_target'] = default_statistics_target
