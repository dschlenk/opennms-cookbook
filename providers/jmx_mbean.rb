...
action :create do
  Chef::Application.fatal!("Missing jmx-collection #{@current_resource.collection_name}.") if !@current_resource.collection_exists
...
def create_jmx_mbean
  ...
  if !mbeans_el.nil?
    ...
    new_resource.attribs.each do |name, details|
      attrib_el = mbean_el.add_element 'attrib', { 'name' => name , 'type' => details['type'], 'alias' => details['alias'] }
    end
  end
