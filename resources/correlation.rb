...
action :create do
  node.default['opennms']['services']['correlator'] = true
  with_run_context(:root) do
    declare_resource(:opennms_send_event, 'restart drools correlation engine') do
      parameters ['daemonName DroolsCorrelationEngine']
      action :nothing
    end
  end unless !find_resource(:opennms_send_event, 'restart drools correlation engine').nil?
  ...
  declare_resource(:execute, "hard restart drools correlation engine #{new_resource.rule_name}") do
  ...
  enames.each do |ename|
    declare_resource(:opennms_send_event, "restart DroolsCorrelationEngine-#{ename}") do
      parameters ["daemonName DroolsCorrelationEngine-#{ename}"]
      action :nothing
    end
  end
  declare_resource(new_resource.engine_source_type.to_sym, dexml) do
