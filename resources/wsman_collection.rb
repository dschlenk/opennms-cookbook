...
action :create do
  converge_if_changed do
    wsman_resource_init
    collection = wsman_resource.variables[:collections][new_resource.collection]
    if collection.nil?
      ...
      wsman_resource.variables[:collections][new_resource.collection] = collection
    else
      run_action(:update)
    end
  end
...
action :delete do
  ...
  unless collection.nil?
    converge_by("Removing wsman_collection #{new_resource.collection}") do
      wsman_resource.variables[:collections].delete(new_resource.collection)
    end
  end
end
